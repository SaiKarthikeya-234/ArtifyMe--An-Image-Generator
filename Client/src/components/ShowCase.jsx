...
function ShowCase({photo, name, _id, prompt, Likes, userName, likedPhotos, setLikedPhotos}) {
  ...
  const LikeImage = async () => {
    if(userName){
      ...
      const req = await fetch(`https://ai-image-backend.netlify.app/.netlify/functions/api/01HGJ91F6Q76EPCPFJKK3MG2DM`, {
        method:'PATCH',
        headers: {
  ...
  return (
    <div className="rounded-xl group relative shadow-card hover:shadow-cardhover card m-2">
      ...
      <div className="mt-5 flex justify-between items-center gap-2">
        ...
        <div className='flex'>
          <p className=" text-white text-sm text-center pr-1.5 mt-0.5"></p>
